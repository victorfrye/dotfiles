...
<#
.SYNOPSIS
    Uploads the local SSH public key to a GitHub account.
.DESCRIPTION
    Registers id_ed25519.pub as both an authentication key and a signing key on GitHub.
    When no token is provided, uses the current gh session (personal account).
    When a token is provided via -Token or GHE_TOKEN, authenticates as that account
...
.EXAMPLE
    Add-GitHubSshKey
.EXAMPLE
    Add-GitHubSshKey -Token $env:GHE_TOKEN -Title 'DESKTOP-WORK'
#>
