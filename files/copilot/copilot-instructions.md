...
- **Durable persistence is required, guided by a per-project contract:** Session/runtime storage may hold a draft, but it is never the final artifact. Always discover the project's retrospective/handoff contract (a doc or labeled section stating where this content belongs); if none exists, propose adding one as an evidence-backed change rather than leaving the record session-private. Once confirmed, commit both the contract (if newly added) and the findings through the project's normal delivery workflow.
